...
def register_redprints(app):
    ...
    app.register_blueprint(user_api, url_prefix='/api/users')  # 注册用户信息管理的 API 接口
    app.register_blueprint(user_department_api, url_prefix='/api/users')  # 注册用户和部门关联的 API 接口
    app.register_blueprint(user_profile_api, url_prefix='/api/user_profile')  # 注册用户个人信息管理的 API 接口
    app.register_blueprint(user_role_api, url_prefix='/api/users')  # 注册用户和角色关联的 API 接口
    app.register_blueprint(visitor_admin_api, url_prefix='/api/visitors_admin')  # 注册管理员的访客信息管理的 API 接口
    app.register_blueprint(visitor_log_admin_api, url_prefix='/api/visitor_logs_admin')  # 注册管理员的访客记录管理的 API 接口
    app.register_blueprint(visitor_user_api, url_prefix='/api/visitors_user')  # 注册普通用户的访客信息管理的 API 接口
    app.register_blueprint(visitor_log_user_api, url_prefix='/api/visitor_logs_user')  # 注册普通用户的预约记录管理的 API 接口
